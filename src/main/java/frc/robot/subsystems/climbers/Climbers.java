...
public class Climbers extends StateMachine<Climbers.State> {
    ...
    public Climbers(ClimberIO leftClimberIO, ClimberIO rightClimberIO) {
        super("Climbers", State.UNDETERMINED, State.class);

        leftClimber = new Climber("Left Climber", leftClimberIO);
        rightClimber = new Climber("Right Climber", rightClimberIO);
    ...
    private Command watchSetpointCommand() {
        return new RunCommand(() -> {
            if (leftClimber.isFlag(Climber.State.AT_SETPOINT) && rightClimber.isFlag(Climber.State.AT_SETPOINT)) {
